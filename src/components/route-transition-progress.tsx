...
function RouteTransitionProgressBar() {
  ...
  return (
    <div
      aria-hidden
      className={cn(
        'bg-primary pointer-events-none fixed inset-x-0 top-0 z-50 h-0.5 origin-left transition-transform duration-300 ease-out',
        transition.active ? 'scale-x-100 opacity-100' : 'scale-x-0 opacity-0',
      )}
    />
  )
}
